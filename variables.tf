...
variable "cvp_cluster_vm_disk_size" {
  description = "The root disk size of the VMs. This must be increased to 40 if running 2021.2.0."
  type        = number
  default     = 35
}
...
variable "gcp_credentials" {
  ...
  validation {
    condition     = var.gcp_credentials == null ? true : fileexists(var.gcp_credentials)
    error_message = "The credentials file does not exist."
  }
}

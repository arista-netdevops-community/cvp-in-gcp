...
resource "null_resource" "cluster_node" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo useradd -m ${var.vm_admin_user} && sudo mkdir -p /home/${var.vm_admin_user}/.ssh && echo \"${var.vm_ssh_key}\" |sudo tee /home/${var.vm_admin_user}/.ssh/authorized_keys && sudo chown -R ${var.vm_admin_user}: /home/${var.vm_admin_user} && sudo echo \"${var.vm_admin_user} ALL=(ALL:ALL) NOPASSWD:ALL\" > /etc/sudoers.d/cvp-sudoers"
